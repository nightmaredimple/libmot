...
# Date   : 17/4/2020
# referring to https://github.com/open-mmlab/mmcv/blob/master/mmcv/utils/config.py
...
from libmot.utils import check_file_exist
...
class Config(object):
    ...
    def _file2dict(filename):
        ...
        if filename.endswith('.py'):
            ...
            raise IOError('Only py/yaml type are supported now!')
        ...
        cfg_dict = Config.check_tuple(cfg_dict)
    ...
    def _merge_a_into_b(a, b):
        ...
        for k, v in a.items():
            if isinstance(v, dict) and k in b and not v.pop('_delete_', False):
    ...
    @staticmethod
    def check_tuple(cfg_dict):
        """Transfer the '(x,x)' like string to tuple(x,x)
        """
        for k, v in cfg_dict.items():
            if isinstance(v, dict):
                v = Config.check_tuple(v)
            else:
